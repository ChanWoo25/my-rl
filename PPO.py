...
class PPO(nn.Module):
    ...
    def put_data(self, transition):
        """
            Remember A Transition with following Format
            (state, action, reward, next_state, previous action prob, done)
        """
    ...
    def train_net(self):
        ...
        for i in range(K_epoch):
            ...
            # To compensate Importance Sampling
            pi = self.pi(s, softmax_dim=1)
            pi_a = pi.gather(1,a)
            ratio = torch.exp(torch.log(pi_a) - torch.log(prob_a))  # a/b == exp(log(a)-log(b))

            # Clip Surrogate Objective func
